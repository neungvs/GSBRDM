...
Public Class TF_FLOATBiz
    ...
    Public Function Count_TF_FLOAT() As Int64
        ...
        sql += "from RDM_StagingDB..TMP_TF_FLOAT FLO "
        sql += "left join RDM_StagingDB..TMP_CU70 cu70 "
        sql += "on  Convert(varchar,CU70.CIF_NUMBER) = FLO.CIF_NUMBER  "
        sql += "left join RDM_VLDDB..REJ_RejectLog rej "
        sql += "on rej.DataSourceTbName like'%TF_FLOAT'  "
        sql += "and rej.DataRecordID = FLO.TMP_TF_FLOATID  "
        ...
        sql += "and (FLO.PrincipalOutstanding<> '' and  FLO.PrincipalOutstanding is not null) "
        sql += "and  (case  when ASSET_CLASS_TYPE in (3,4,5) then  "
        sql += "( case when ISNUMERIC (PrincipalOutstanding)= 1    then  CAST( ISNULL(PrincipalOutstanding,0)   as decimal(24,0) )   end) "
        sql += "else   ( case when ISNUMERIC (PrincipalOutstanding)= 1    then  CAST( ISNULL(PrincipalOutstanding,0)   as decimal(24,0) )   end) "
        sql += "+ case when ISNUMERIC (AccruedInterest)= 1  then  CAST( ISNULL(AccruedInterest,0)   as decimal(24,0) )   else 0   end  "
    ...
    Public Function GetDataTF_FLOAT() As DataTable
        ...
        sql = "Select  tf.TMP_TF_FLOATID,tf.PositionID,tf.CIF_Number "
        sql += ",tf.CustName ,tf.Status,tf.StartDate ,tf.EndDate "
        sql += ",tf.Currency_Original ,tf.PrincipalOutstanding,tf.AccruedInterest  "
        sql += ",tf.Coupon_Rate ,tf.ASSET_CLASS_TYPE,AS_OF_DATE  "
        sql += "FROM(select flo.TMP_TF_FLOATID,flo.CIF_NUMBER   "
        sql += ",flo.PositionID ,ISNULL(cu70.CUST_NAME,'') "
        sql += "+' '+ ISNULL(cu70.CUST_SURNAME,'')  as CustName ,Status ,flo.StartDate "
        sql += ",flo.EndDate,flo.PrincipalOutstanding  "
        sql += ",flo.AccruedInterest,Currency_Original  "
    ...
    Public Function GetTF_FLOAT_ByCriteria(ByVal cifNum As String, ByVal accNo As String, ByVal cifName As String, ByVal cifSur As String) As DataTable
    ...
    Public Function GetRejData(ByVal rejId As String) As List(Of RejLeistEntity)
        ...
        sql += "WHERE [DataSourceTbName] = 'TMP_TF_FLOAT' "
        ...
        For Each rs As DataRow In dt.Rows
            ...
            sql += " as TMP_Values "
            sql += "FROM  RDM_StagingDB..TMP_TF_FLOAT "
            sql += "WHERE TMP_TF_FLOATID = '" + rejId + "' "
    ...
    Public Function GetRejData_Report() As List(Of RejListReportEntity)
        ...
        sql += ",(select CIF_Number from [RDM_StagingDB]..TMP_TF_FLOAT where TMP_TF_FLOATID = rej.DataRecordID) CifNumber "
        sql += ",(select PositionID  from [RDM_StagingDB]..TMP_TF_FLOAT where TMP_TF_FLOATID = rej.DataRecordID) AccNo "
        sql += "FROM [RDM_VLDDB]..[REJ_RejectLog] rej "
        sql += "inner join RDM_StagingDB..TMP_TF_FLOAT float "
        sql += "on rej.DataSourceTbName LIKE '%TF_FLOAT' "
        sql += "and rej.DataRecordID = float.TMP_TF_FLOATID "
        ...
        For Each rs As DataRow In dt.Rows
            ...
            sql += " as TMP_Values "
            sql += "FROM  RDM_StagingDB..TMP_TF_FLOAT "
            sql += "WHERE TMP_TF_FLOATID = '" + rs("DataRecordID").ToString() + "' "
    ...
    Public Sub Save_edit(ByVal TMP_TF_FLOATID As String, ByVal columnName As String, ByVal columnValues As String)

        Try

            Dim sql As String = String.Empty

            sql = "UPDATE RDM_StagingDB..TMP_TF_FLOAT "
            sql += "SET " + columnName + " = " + "'" + columnValues + "' "
            sql += "where TMP_TF_FLOATID  = " + TMP_TF_FLOATID

            dbCon.ExecuteNonQuery(sql)

            Insert_HistoryTF_FLOAT(TMP_TF_FLOATID, columnName)
    ...
    Private Sub Insert_HistoryTF_FLOAT(TMP_TF_FLOATID As Object, columnName As String)
        ...
        Try
            ...
            sql += "VALUES( " + " 'TMP_TF_FLOAT'," + TMP_TF_FLOATID + ", 1  ,  getdate() ,'" + columnName + "') "
