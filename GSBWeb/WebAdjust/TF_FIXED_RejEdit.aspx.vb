...
Public Class TF_FIXED_RejEdit
    ...
    Private Sub GetRejData(tempId As String)
        ...
        gvTF_FIXED.DataSource = tfBiz.GetRejData(tempId)
        gvTF_FIXED.DataBind()
    ...
    Protected Sub gvTF_FIXED_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvTF_FIXED.SelectedIndexChanged
        ...
        Dim HF_DataRecordID As HiddenField = CType(gvTF_FIXED.SelectedRow.FindControl("HF_DataRecordID"), HiddenField)

        Dim lblColumnName As Label = CType(gvTF_FIXED.SelectedRow.FindControl("lblColumnName"), Label)

        Dim lblColumnValue As Label = CType(gvTF_FIXED.SelectedRow.FindControl("lblColumnValue"), Label)

        Dim lblRejectReason As Label = CType(gvTF_FIXED.SelectedRow.FindControl("lblRejectReason"), Label)

        Dim lblSeq As Label = CType(gvTF_FIXED.SelectedRow.FindControl("lblSeq"), Label)


        Dim HF_ColumnName As HiddenField = CType(gvTF_FIXED.SelectedRow.FindControl("HF_ColumnName"), HiddenField)
    ...
    Private Sub SetValidation(columnName As String)
        ...
        ElseIf (New String() {"Coupon_Rate"}).Contains(columnName) Then
            ...
            rgFormat.ValidationExpression = "^(?:[1-9][0-9]{0,3}|(?![0-9.]{8})(?:0\.[0-9]*[1-9][0-9]*|[1-9][0-9]*\.[0-9]+))$"
    ...
    Protected Sub btnDetail_Click(sender As Object, e As EventArgs) Handles btnDetail.Click
        ...
        lblModalTitle.Text = "คำอธิบายรูปแบบข้อมูลบัญชีเงินกู้ในธุรกิจ Trade Finance ที่มีอัตราดอกเบี้ยคงที่ -> TF_FIXED"
    ...
    Protected Sub btnClose_Click(sender As Object, e As EventArgs) Handles btnClose.Click
        ...
        Response.Redirect("TF_FIXED_RejList.aspx")
