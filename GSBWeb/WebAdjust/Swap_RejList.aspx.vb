...
Public Class Swap_RejList
    ...
    Private Sub LoadData()
        ...
        gvSwap.DataSource = swapBiz.GetDataSWAP()
        gvSwap.DataBind()
    ...
    Protected Sub gvSwap_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvSwap.SelectedIndexChanged
        ...
        Dim lblCifNumber As Label = CType(gvSwap.SelectedRow.FindControl("lblCifNumber"), Label)
        Dim hpPositionID As LinkButton = CType(gvSwap.SelectedRow.FindControl("hpPositionID"), LinkButton)
        Dim HF_TMP_SwapID As HiddenField = CType(gvSwap.SelectedRow.FindControl("HF_TMP_SwapID"), HiddenField)

        ClearSession()
        Session("CifNumber") = lblCifNumber.Text
        Session("ACCNO") = hpPositionID.Text
        Session("TMPID") = HF_TMP_SwapID.Value

        Response.Redirect("Swap_RejEdit.aspx")

    End Sub

    Protected Sub gvSwap_PageIndexChanging(sender As Object, e As System.Web.UI.WebControls.GridViewPageEventArgs) Handles gvSwap.PageIndexChanging
    ...
    Protected Sub gvSwap_RowDataBound(sender As Object, e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvSwap.RowDataBound
        ...
        Dim HF_TMP_SwapID As HiddenField = CType(e.Row.FindControl("HF_TMP_SwapID"), HiddenField)
        ...
        If (e.Row.RowType = DataControlRowType.DataRow) Then
            ...
            HF_TMP_SwapID.Value = DataBinder.Eval(e.Row.DataItem, "TMP_SwapID").ToString()
    ...
    Private Sub SearchData()
        ...
        gvSwap.DataSource = swapBiz.GetFW53ByCriteria(Convert.ToString(Session("CifNum")), Convert.ToString(Session("AccNo")), Convert.ToString(Session("Name")), Convert.ToString(Session("SurName")))
        gvSwap.DataBind()
    ...
    Protected Sub btnReport_Click(sender As Object, e As EventArgs) Handles btnReport.Click
        ...
        lsRej = swapBiz.GetRejData_Report()


        For Each item As RejListReportEntity In lsRej

            If item.ColumnName = "ASSET_GL_CODE" Then

                item.ColumnName = "GL Account กรณีที่สัญญาอนุพันธ์ (ASSET_GL_CODE)"

            ElseIf item.ColumnName = "AS_OF_DATE" Then
                item.ColumnName = "วันที่ของข้อมูล (AS_OF_DATE)"

            ElseIf item.ColumnName = "STARTDATE" Then
                item.ColumnName = "วันที่เริ่มต้นของรายการ (STARTDATE)"

            ElseIf item.ColumnName = "ENDDATE" Then
                item.ColumnName = "เป็นข้อมูลวันที่ครบกำหนดสัญญา (ENDDATE)"
        ...
        viewer.LocalReport.ReportPath = Server.MapPath("~/Reports/Swap_RejReport.rdlc")
        ...
        Dim fileName As String = "Swap_RejectDetail_" + String.Format("{0:ddMMyyyy}", Date.Now) + ".pdf"
