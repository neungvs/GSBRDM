...
Public Class TF_FIXED_RejList
    ...
    Private Sub LoadData()
        ...
        gvTF_FIXED.DataSource = tfBiz.GetDataTF_FIXED()
        gvTF_FIXED.DataBind()
    ...
    Protected Sub gvTF_FIXED_RowDataBound(sender As Object, e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvTF_FIXED.RowDataBound

        Dim hpPositionId As LinkButton = CType(e.Row.FindControl("hpPositionId"), LinkButton)
        Dim HF_TMP_TF_FIXEID As HiddenField = CType(e.Row.FindControl("HF_TMP_TF_FIXEID"), HiddenField)
        ...
        If (e.Row.RowType = DataControlRowType.DataRow) Then
            ...
            HF_TMP_TF_FIXEID.Value = DataBinder.Eval(e.Row.DataItem, "TMP_TF_FIXEID").ToString()
    ...
    Protected Sub gvTF_FIXED_PageIndexChanging(sender As Object, e As System.Web.UI.WebControls.GridViewPageEventArgs) Handles gvTF_FIXED.PageIndexChanging

        gvTF_FIXED.PageIndex = e.NewPageIndex
    ...
    Private Sub SearchData()
        ...
        gvTF_FIXED.DataSource = tfBiz.GetTF_FIXED_ByCriteria(Convert.ToString(Session("CifNum")), Convert.ToString(Session("AccNo")), Convert.ToString(Session("Name")), Convert.ToString(Session("SurName")))
        gvTF_FIXED.DataBind()
    ...
    Protected Sub gvTF_FIXED_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvTF_FIXED.SelectedIndexChanged


        Dim lblCifNumber As Label = CType(gvTF_FIXED.SelectedRow.FindControl("lblCifNumber"), Label)
        Dim hpPositionId As LinkButton = CType(gvTF_FIXED.SelectedRow.FindControl("hpPositionId"), LinkButton)
        Dim HF_TMP_TF_FIXEID As HiddenField = CType(gvTF_FIXED.SelectedRow.FindControl("HF_TMP_TF_FIXEID"), HiddenField)

        ClearSession()
        Session("CifNumber") = lblCifNumber.Text
        Session("ACCNO") = hpPositionId.Text
        Session("TMPID") = HF_TMP_TF_FIXEID.Value

        Response.Redirect("TF_FIXED_RejEdit.aspx")
    ...
    Protected Sub btnReport_Click(sender As Object, e As EventArgs) Handles btnReport.Click
        ...
        viewer.LocalReport.ReportPath = Server.MapPath("~/Reports/TF_FIXED_RejReport.rdlc")
        ...
        Dim fileName As String = "TF_FIXED_RejDetail_" + String.Format("{0:ddMMyyyy}", Date.Now) + ".pdf"
