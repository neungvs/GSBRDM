...
Public Class Industry_Limit_Add
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Page.IsPostBack = False Then
            SetControl()
            hfEffectiveDate.Value = Convert.ToString(Session("effectiveDate"))
        End If
    End Sub

    Protected Sub btnCancle_Click(sender As Object, e As EventArgs) Handles btnCancle.Click
        Session("effectiveDate") = hfEffectiveDate.Value
        ClearDetail()
    ...
    '        'Check ISICODE
    '        'If (ddlISICCODE.SelectedIndex = 0 And txtSearchMkt.Text = String.Empty) Then
    '        '    UtilsBiz.CreateMessageAlert(Page, "กรุณาเลือก ISICCODE หรือ ISICCODEMKTLEVEL", gui)
    '        '    Return
    '        'End If
    '        'If (ddlISICSUB.SelectedIndex <> 0) Then
    '        '    ISICCODESUBLEVEL = ddlISICSUB.SelectedValue
    '        'End If
    ...
    '            UtilsBiz.CreateMessageAlert(Page, "กรุณาบันทึกข้อมูล เปอร์เซ็น หรือ วงเงินอนุมัติ อย่างใดอย่างหนึ่ง", gui)
    '            Return
    '        End If
    ...
    '        If ddlISICSUB.SelectedIndex <> -1 Then
    '            chkISIC = lmBiz.CheckISIC(ddlISICCODE.SelectedItem.ToString(), ISICCODESUBLEVEL, Convert.ToString(HF_effectiveDate.Value))
    '            If chkISIC = False Then
    '                UtilsBiz.CreateMessageAlert(Page, "ไม่สามารถบันทึกข้อมูลได้เนื่องมีข้อมูล ISICCODE และ ISICCODESUBLEVEL อยู่แล้ว", gui)
    '                Return
    '            End If
    '        End If

    '        'If txtSearchMkt.Text <> String.Empty Then
    '        '    chkISIC = lmBiz.CheckLnCode(txtSearchMkt.Text, Convert.ToString(HF_effectiveDate.Value))
    '        '    If chkISIC = False Then
    '        '        UtilsBiz.CreateMessageAlert(Page, "ไม่สามารถบันทึกข้อมูลได้เนื่องมีข้อมูล ISICCODEMKTLEVEL อยู่แล้ว", gui)
    '        '        Return
    '        '    End If
    '        'End If

    '        If lblIndustry.Text = "ไม่พบข้อมูล" Then
    '            UtilsBiz.CreateMessageAlert(Page, "ไม่พบข้อมูล ISICCODEMKTLEVEL", gui)
    '            Return
    '        End If


    '        Dim issicCode As String

    '        If (ddlISICSUB.SelectedIndex <> -1) Then
    '            issicCode = ddlISICCODE.SelectedItem.ToString()
    '        Else
    '            issicCode = String.Empty
    '        End If

    '        ' lmBiz.InsertDetail(issicCode, ISICCODESUBLEVEL, lblIndustry.Text, Convert.ToString(IndustryLimitPercentage), Convert.ToString(IndustryLimitAmount), Convert.ToString(HF_effectiveDate.Value), txtSearchMkt.Text)
    '        Session("effectiveDate") = HF_effectiveDate.Value

    '        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "alert",
    '       "alert('บันทึกข้อมูลสำเร็จ');window.location ='ModifyIndustryLimit.aspx';", True)
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        Try
            ...
            Dim ent As New IndustyLimitDetailEntity

            ent = GetDataDetail(entDetail, hfEffectiveDate.Value)

            Dim _result As Boolean

            _result = lmBiz.InsertDetail(ent)
    ...
    Private Function GetDataDetail(ByVal _entDetail As IndustyDetailEntity, ByVal _effectiveDate As String) As IndustyLimitDetailEntity

        Dim _Result As New IndustyLimitDetailEntity

        If _entDetail.Type = 1 Then
            _Result.Level1 = _entDetail.ISICCODE
            _Result.Level2 = _entDetail.ISICCODESUBLEVEL
            _Result.Level3 = ""
            _Result.IndustryDesc = _entDetail.Industry
            _Result.LoantypeDesc = ""
        Else
            _Result.Level1 = _entDetail.LN_TYPE_CODE
            _Result.Level2 = _entDetail.LN_SUB_TYPE
            _Result.Level3 = _entDetail.LN_MKT_CODE
            _Result.IndustryDesc = ""
            _Result.LoantypeDesc = _entDetail.LoanType
        End If

        _Result.Type = _entDetail.Type
        _Result.IndustryLimitAmount = _entDetail.IndustryLimitAmount
        _Result.IndustryLimitPercentage = _entDetail.IndustryLimitPercentage
        _Result.EffectiveDate = _effectiveDate
